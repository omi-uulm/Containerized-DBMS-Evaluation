...
#installation and setup script for cassandra monitoring
#NOTE: make sure you have the jolokia-jvm.jar binded in your cassandra-env.sh 
...
#infludb database name, choose according to benchmark: bare, bare_docker, vm, vm_docker, SCDM
influx_database="SCDM"
...
#cassandra
echo '[[inputs.cassandra]]' >> ${config_filename}
echo 'context = "/jolokia/read" ' >> ${config_filename}
echo 'servers = ["'${cassandra_user}':'${cassandra_password}'@:8778"] ' >> ${config_filename}
echo 'metrics = ["/org.apache.cassandra.metrics:type=Client,name=connectedNativeClients","/org.apache.cassandra.metrics:type=Storage,name=Load","/java.lang:type=Memory/HeapMemoryUsage","/java.lang:type=Memory/NonHeapMemoryUsage","/org.apache.cassandra.metrics:type=ClientRequest,scope=Read,name=Latency","/org.apache.cassandra.metrics:type=ClientRequest,scope=Write,name=Latency","/org.apache.cassandra.metrics:type=CommitLog,name=PendingTasks"] ' >> ${config_filename}
