...
function checkDirectory {
    docker cp check_directory.sh ycsb_vm:/opt/check_directory.sh
    docker exec -i ycsb_vm /bin/bash -c "./check_directory.sh $1 $2 $3"
}

## creates TestDirectory for timestamp in
## $1 threadcount, $2 TIMESTAMP, $3 IS_DOCKER, $4 HAS_ATTACHED_VOL
function createTestDirectory {
    docker cp create_test_directory.sh ycsb_vm:/opt/create_test_directory.sh
    docker exec -i ycsb_vm /bin/bash -c "./create_test_directory.sh $1 $2 $3 $4"
}
...
function loadWorkload {
    ...
    echo -e "cd /opt/ycsb-0.12.0\n./bin/ycsb load mongodb -s -threads $1 -P workloads/workloada -p recordcount=2000000 -p operationcount=10000000 -threads 8 -p mongodb.url='mongodb://127.0.0.1:${MONGO_PORT}/ycsb?w=1' > /opt/test_results/${3}/load.txt 2>&1" > load_workload.sh
    echo "start load docker exec"
...
function runWorkloadA {
    ...
    echo -e "cd /opt/ycsb-0.12.0\n./bin/ycsb run mongodb -s -threads $1 -P workloads/workloada -p recordcount=2000000 -p operationcount=10000000 -threads 8 -p mongodb.url='mongodb://127.0.0.1:${MONGO_PORT}/ycsb?w=1' > /opt/test_results/${3}/run_workloadA.txt 2>&1" > run_workloadA.sh
...
function runWorkloadB {
    ...
    echo -e "cd /opt/ycsb-0.12.0\n./bin/ycsb run mongodb -s -threads $1 -P workloads/workloadb -p recordcount=2000000 -p operationcount=10000000 -threads 8 -p mongodb.url='mongodb://127.0.0.1:${MONGO_PORT}/ycsb?w=1' > /opt/test_results/${3}/run_workloadB.txt 2>&1" > run_workloadB.sh
    ...
    do
        ...
        do 
            ...
            if [ $IS_DOCKER = false ]; then
                TESTPATH="mongodb_ycsb_tests/vm/${var}_threads/${TIMESTAMP}"
            else
                if [ $HAS_ATTACHED_VOL = true ]; then
                    TESTPATH="mongodb_ycsb_tests/dockered_attached_vol/${var}_threads/${TIMESTAMP}"
                else
                    TESTPATH="mongodb_ycsb_tests/dockered/${var}_threads/${TIMESTAMP}"
                fi
            fi
            #not for baremetal
